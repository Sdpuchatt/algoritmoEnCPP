...
void cargarDatos(tEmpresa &nicTech);
void guardarDatos(const tEmpresa &nicTech);
...
int main() {
    ...
    // Guardar datos de vuelta al archivo al salir del programa
    guardarDatos(nicTech);
...
void cargarDatos(tEmpresa &nicTech) {
    ...
    if (archivo.is_open()) {
        archivo >> nicTech.contador;  // Leer el contador de clientes
        for (int i = 0; i < nicTech.contador; ++i) {
            archivo >> nicTech.cliente[i].codigo;
            archivo >> nicTech.cliente[i].nombre;
            archivo >> nicTech.cliente[i].apellido;
        }
        archivo.close();
        cout << "Datos cargados desde db.txt exitosamente.\n";
    } else {
        cout << "No se pudo abrir el archivo db.txt. Se comenzará con datos vacíos.\n";
    }
}
...
void guardarDatos(const tEmpresa &nicTech) {
    ...
    if (archivo.is_open()) {
        archivo << nicTech.contador << endl;  // Guardar el contador de clientes
        for (int i = 0; i < nicTech.contador; ++i) {
            archivo << nicTech.cliente[i].codigo << " ";
            archivo << nicTech.cliente[i].nombre << " ";
            archivo << nicTech.cliente[i].apellido << endl;
        }
        archivo.close();
        cout << "Datos guardados en db.txt exitosamente.\n";
    } else {
        cout << "No se pudo abrir el archivo db.txt para guardar los datos.\n";
    }
}
