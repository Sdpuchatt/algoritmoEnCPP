...
{
    CustomerDatabase db;
    db.numCustomers = 0; // Inicializo en cero el contador
    
    db.numCustomers = loadCustomersFromFile(db); // cargo la lista de cliente
...
void create(CustomerDatabase &db) 
{
    ...
    {
        // newCustomer.code = "C" + to_string(db.numCustomers + 101);
        string codeStr = db.customers[db.numCustomers-1].code;
        // convierto a entero substring evito la c y me quedo con el resto
        int codigo = stoi(codeStr.substr(1))+1;
        string codigoFinal = to_string(codigo);
        newCustomer.code = "c" + codigoFinal;
         
    } else 
    {
        newCustomer.code = "c100"; // Comienza en C100 si es el primer cliente
    }
